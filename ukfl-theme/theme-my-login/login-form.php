...
<div class="tml tml-login" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	<form name="loginform" id="loginform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'login', 'login_post' ); ?>" method="post">
		<p class="tml-user-login-wrap form-group">
			<label class="control-label" for="user_login<?php $template->the_instance(); ?>"><?php
				if ( 'username' == $theme_my_login->get_option( 'login_type' ) ) {
					_e( 'UKFL Number', 'theme-my-login' );
				} elseif ( 'email' == $theme_my_login->get_option( 'login_type' ) ) {
					_e( 'E-mail', 'theme-my-login' );
				} else {
					_e( 'UKFL Number or E-mail', 'theme-my-login' );
				}
			?></label>
		...
		<p class="tml-user-pass-wrap form-group">
			<label class="control-label" for="user_pass<?php $template->the_instance(); ?>"><?php _e( 'Password', 'theme-my-login' ); ?></label>
		...
		<div class="tml-rememberme-submit-wrap">
			...
			<p class="tml-submit-wrap">
				<input type="submit" class="btn btn-success" name="wp-submit" id="wp-submit<?php $template->the_instance(); ?>" value="<?php esc_attr_e( 'Log In', 'theme-my-login' ); ?>" />
